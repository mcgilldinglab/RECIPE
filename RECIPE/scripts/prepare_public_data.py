...
def _file_status(data_root: Path, relative_path: str) -> dict[str, Any]:
...
def main() -> None:
    ...
    missing = [item for item in required_status if not item["exists"] or item["size_bytes"] == 0]
    summary["required_inputs"] = required_status
    summary["missing_required_inputs"] = missing
    summary["ready"] = not missing
